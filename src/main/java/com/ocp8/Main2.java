...
public class Main2 {
    public static void main(String[] args) {
        ...
        if(success){
            System.out.printf("租車 %d 台成功, 租金: $%,d", rentCarAmount, rc.getRent());
        } else {
            System.out.printf("租車 %d 台失敗(庫存不足)\n", rentCarAmount);
        }
        if(success2){
            System.out.printf("租腳踏車 %d 台成功, 租金: $%,d", rentCarAmount, rc.getRent());
        } else {
            System.out.printf("租腳踏車 %d 台失敗(庫存不足)\n", rentCarAmount);
        }
        if(success3){
            System.out.printf("租飛機 %d 台成功, 租金: $%,d", rentCarAmount, rc.getRent());
        } else {
            System.out.printf("租飛機 %d 台失敗(庫存不足)\n", rentCarAmount);
        }
        // 列印庫存
        rc.print();
    }
